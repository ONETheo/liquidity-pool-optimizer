...
This script calculates and visualizes a Bitcoin volatility index based on collected price data.
...
BTC_DAILY_FILE = os.path.join(DATA_DIR, "BTC_daily.csv")
BTC_HOURLY_FILE = os.path.join(DATA_DIR, "BTC_hourly.csv")


class BitcoinVolatilityIndex:
    """
    Calculate and track Bitcoin volatility using multiple methodologies.
    """
    ...
    def __init__(self):
        ...
        # Load data
        self.daily_data = self._load_data(BTC_DAILY_FILE)
        self.hourly_data = self._load_data(BTC_HOURLY_FILE)
    ...
    def calculate_hourly_volatility(self):
        ...
        # Equal weight to std deviation and average absolute change
        df['btc_volatility_index_hourly'] = (
    ...
    def save_volatility_data(self):
        ...
        if self.daily_volatility is not None:
            daily_file = os.path.join(OUTPUT_DIR, "BTC_daily_volatility.csv")
        ...
        if self.hourly_volatility is not None:
            hourly_file = os.path.join(OUTPUT_DIR, "BTC_hourly_volatility.csv")
    ...
    def plot_daily_volatility(self):
        ...
        # Plot price
        self.daily_volatility['price'].plot(ax=ax1, color='blue', label='BTC Price')
        
        # Plot volatility index
        self.daily_volatility['btc_volatility_index'].plot(
            ax=ax1_twin, color='red', label='BTC Volatility Index', linewidth=2
        )
        
        ax1.set_title('Bitcoin Price and Volatility Index (Daily)', fontsize=14)
        ...
        ax2.set_title('Bitcoin Return Volatility (Different Time Windows)', fontsize=14)
        ...
        ax3.set_title('Bitcoin Average Daily Price Change (%)', fontsize=14)
        ...
        # Save plot
        plot_file = os.path.join(OUTPUT_DIR, "BTC_daily_volatility.png")
    ...
    def plot_hourly_volatility(self):
        ...
        # Plot price
        self.hourly_volatility['price'].plot(ax=ax1, color='blue', label='BTC Price')
        
        # Plot volatility index
        self.hourly_volatility['btc_volatility_index_hourly'].plot(
            ax=ax1_twin, color='red', label='BTC Hourly Volatility Index', linewidth=2
        )
        
        ax1.set_title('Bitcoin Price and Volatility Index (Hourly)', fontsize=14)
        ax1.set_ylabel('Price (USD)', color='blue')
        ax1_twin.set_ylabel('Volatility Index', color='red')
        ...
        # Save plot
        plot_file = os.path.join(OUTPUT_DIR, "BTC_hourly_volatility.png")
        plt.savefig(plot_file, dpi=300)
...
def main():
    ...
    # Initialize the volatility index calculator
    btc_volatility = BitcoinVolatilityIndex()
    
    # Calculate daily volatility
    print("Calculating daily volatility metrics...")
    btc_volatility.calculate_daily_volatility()
    
    # Calculate hourly volatility
    print("Calculating hourly volatility metrics...")
    btc_volatility.calculate_hourly_volatility()
    
    # Save data to CSV files
    print("Saving volatility data to CSV files...")
    output_files = btc_volatility.save_volatility_data()
    
    # Create plots
    print("Creating volatility plots...")
    daily_plot = btc_volatility.plot_daily_volatility()
    hourly_plot = btc_volatility.plot_hourly_volatility()
